...
**Plans**: 9/9 plans executed — 6/6 executed, plus 3 gap-closure plans from UAT
...
- [x] 08-05-PLAN.md — The summary mirror tick-box and the third printed sheet (wave 4)
...
- [x] 08-09-PLAN.md — The order form prints exactly its sheets on Letter at 100% (G-08-10)
...
- [x] 09-02-PLAN.md — Tracer: TEMPLATE works on a phone end to end — the two switches, the dvh root, the shared shell, the phone bars (wave 2)
...
**Plans**: 11/11 plans executed — 4/4 executed in waves 1-2; 3 gap-closure plans executed in waves 3-4; the real-device sweep (wave 7) and the three fixes its first walk produced (waves 5-6) are outstanding

Plans:
**Wave 1**

- [x] 10-01-PLAN.md — The rack's dialogs, thumb-sized: the close X and the board-name field (wave 1)
- [x] 10-02-PLAN.md — The account control on a phone: the Sign in row, Clerk's avatar, the banner's dismiss X (wave 1)
- [x] 10-03-PLAN.md — The home screen: the tab bar steps aside, one shared thumbnail, a card that fits (wave 1)
...
- [x] 10-10-PLAN.md — A phone on its side goes back to being a normal browser, and the Hide Toolbar tip finds the place it belongs (wave 6)
- [x] 10-11-PLAN.md — The rails instructions page stops carrying sliders that do nothing (wave 6)

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 10-08-PLAN.md — The real-device sweep, re-issued and walked from step 1 on both phones (wave 7)

Cross-cutting constraints:

- Every fix is gated on the `coarse` pointer variant, the width variant or a route check, so a desktop mouse sees an unchanged screen; the desktop screenshot baselines are the regression proof and are never re-recorded. One fix in the second gap-closure set (10-11) adds an inert wrapper element, so the markup is not byte-identical there — what is unchanged, and is asserted, is everything a mouse actually sees.
- Wave 1's three plans touch sixteen files with no overlap, so they run in parallel worktrees.
- Gap closure: wave 3 is the layout switch and lands alone, because it silently re-flows every use of the phone/desktop variants across the codebase. Wave 4's two plans depend on it and touch no file in common. The sweep is human work on real phones and cannot start until every code fix has landed.
- Second gap closure (2026-09-11, from the first real-device walk): wave 6 lands the board card's floor first, because it is the only one of the three fixes that is correct both before and after the sideways decision — it moves the card's height cap off the layout axis onto the pointer axis, so nothing about the layout revert can reach it. Wave 7 then reverts the layout switch (D-10) and fixes the rails instructions page; the two touch no file in common. Wave 8 is the re-walked sweep, from step 1 on both phones, because D-10 changes sideways behaviour on every screen and nothing walked on the old build still stands.
