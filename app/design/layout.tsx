 ...
 * layout, so this file's four jobs are the height-passthrough wrapper div below — it carries the
 ...
 * all), and mounting `PhoneTabBar` (D-06/D-07) as the LAST child, below `props.children`, so it
 * appears on the same six design routes and nowhere else. The banner and the tip are both
 * `flex-none`; each editor already declares `flex-1`/`min-h-0` on its own root, so adding any bar
 * here does not disturb that sizing chain — `PhoneTabBar` paints or not purely from its own
 * `max-shell:` CSS variant, with no layout impact on a desktop width.
 */
